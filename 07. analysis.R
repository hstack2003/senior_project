...
subject_age_did_results <- istep_did_data |> 
  ...
  mutate(mod = map(data, 
                   ~ feols(`Proficient %` ~ treated * post_treat | 
                             IDOE_SCHOOL_ID + SCHOOL_YEAR_ID + GRADE_CODE,
...
#                             levels = c("All Grades", "Elementary", "Middle", "High School"))) |>
#   filter(
#     !is.na(white_level_hi_lo)) |>
#   nest(data = -c(subject, white_level_hi_lo, age_range)) |>
#   mutate(mod = map(data,
#                    ~ feols(`Proficient %` ~ treated * post_treat |
...
#                            data = .x)),
#          coefs = map(mod, broom::tidy)) |>
#   select(subject, white_level_hi_lo, age_range, coefs) |>
#   unnest(cols = coefs) |>
#   group_by(subject) |>
#   mutate(p_adj_holm = p.adjust(p.value, method = "holm")) |>
#   ungroup() |>
#   arrange(age_range, white_level_hi_lo)
# 
